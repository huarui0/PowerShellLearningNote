...
2. 根据源文件夹
#>
function local:CreateDestinationEmptyFolder {
    <#
    .SYNOPSIS
        实现：
...
function global:CopySourceFolderToDestination {
    <#
    .SYNOPSIS
        实现：
        1. 将源文件夹拷贝到指定的目标文件夹下（在目标文件夹下创建了新的文件夹）。
...
function script:Sub_CopySourceFolderToDestination {
    <#
    .SYNOPSIS
        实现：
        1. 将源文件夹拷贝到指定的目标文件夹下（在目标文件夹下创建了新的文件夹）。
...
# 2. 测试 - 将文件夹Copy到ParentFolder下，并更名 - 更名不能直接拷贝文件夹，用 -isCopyFolderContents 的方法 -- 不用也可以，但要有 -NewDistinationFolderName
                CopySourceFolderToDestination -TemplateFolder $TemplateFolder -SourceParentFolder $SourceParentFolder -DistinationParentFolder $DistinationParentFolder -ForceOverride:$false -NewDistinationFolderName $NewDistinationFolderName -isCopyFolderContents:$true
