...
public class Activity_Detail_IO2014 extends AppCompatActivity {
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_android_io_2014_detail);

        Bitmap photo = setupPhoto(getIntent().getIntExtra("photo", R.drawable.album1));
        ...
        getWindow().getEnterTransition().addListener(new TransitionAdapter() {
            ...
            public void onTransitionEnd(Transition transition) {
                ImageView hero = (ImageView) findViewById(R.id.hero_photo);
...
/*
    private void setupMap() {
        GoogleMap map = ((MapFragment) getSupportFragmentManager().findFragmentById(R.id.map)).getMap();
...
*/
    ...
    private void applyPalette(Palette palette) {
        int darkMutedColor = Color.rgb(255,140,0);    // Color.YELLOW
        getWindow().setBackgroundDrawable(new ColorDrawable(palette.getDarkMutedColor(darkMutedColor)));

        int vibrantColor = Color.rgb(255,69,0);  // Color.YELLOW
        TextView titleView = (TextView) findViewById(R.id.title);
        titleView.setTextColor(palette.getVibrantColor(vibrantColor));

        TextView descriptionView = (TextView) findViewById(R.id.description);
        descriptionView.setTextColor(palette.getLightVibrantColor(vibrantColor));

        colorRipple(R.id.info, palette.getDarkMutedColor(vibrantColor),
                palette.getDarkVibrantColor(vibrantColor));
        colorRipple(R.id.star, palette.getMutedColor(vibrantColor),
                palette.getVibrantColor(vibrantColor));

        View infoView = findViewById(R.id.information_container);
        infoView.setBackgroundColor(palette.getLightMutedColor(vibrantColor));

        // AnimatedPathView star = (AnimatedPathView) findViewById(R.id.star_container);
        // star.setFillColor(palette.getVibrantColor(Color.DKGRAY));
        // star.setStrokeColor(palette.getLightVibrantColor(Color.DKGRAY));
    }
    ...
    private Bitmap setupPhoto(int resource) {
        Bitmap bitmap = Activity_Main_IO2014.sPhotoCache.get(resource);
        ((ImageView) findViewById(R.id.hero_photo)).setImageBitmap(bitmap);
        return bitmap;
    }
/*
...
*/
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.menu_android_io_2014_detail, menu);
        return true;
    }
}
