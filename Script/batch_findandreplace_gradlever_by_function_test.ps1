...
Break    
                        ...
                        Get-ChildItem -Path $FilePath -Recurse | Where-Object -Property name -eq -Value $using:FileName | ForEach-Object -Parallel {
                            ...
                            if (Test-Path -Path $_ -PathType 'Leaf') {
                                # 判断是否是 原始文件拷贝到目标文件方式
                                #Write-Host "第二层循环_开始1: `$using:OriginalFilePath = $using:OriginalFilePath"
                                #if ($using:OriginalFilePath) {
                                    # 用 OriginalFilePath 判断 是否将 OriginalFilePath 中的原文件，拷贝到目标位置
                                    # 如果是强制覆盖

                                    if ($ForceOverride.IsPresent) {
                                        #if ($using:TempFileMode.IsPresent) {
                                            # --- 要移到前面去，不能放这里！--- #
                                            ...
                                            #Copy-Item -Path "$using:SourceFilePath.tmp" -Destination $using:FilePath
                                ...
                                if (Test-Path -Path $DestinationFilePath -PathType 'Leaf') {
                                    ...
                                    if ($ForceOverride.IsPresent) {
                                            ...
                                            # 考虑到其他文件还要用，不能用Move-Item，而要用Copy-Item的方式，待全部处理完后，统一删除临时的文件。
                                            # Move-Item -Path "$using:SourceFilePath.tmp" -Destination $using:FilePath
                                            # Copy-Item -Path "$using:SourceFilePath.tmp" -Destination $using:FilePath
                                        #} else {
            ...
            if ($using:OriginalFilePath) {
                if ($using:TempFileMode.IsPresent) {
                    if (Test-Path -Path "$SourceFilePath.tmp" -PathType 'Leaf') {
                        Remove-Item -Path "$SourceFilePath.tmp" -Force
                    }
                } else {
